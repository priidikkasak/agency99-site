...
export async function POST(request: Request) {
  try {
    ...
    const typesArr: string[] = Array.isArray(sourcingTypes)
    ...
    const safe = {
      ...
      sourcingTypes: typesArr.map((v) => clean(v, 100)).join(', '),
      otherDescription: clean(otherDesc, 500),
      targetMarkets: clean(targetMarkets, 500),
    ...
    await resend.emails.send({
      ...
      replyTo: safe.email,
      subject: `Sourcing questionnaire - ${safe.name} (${safe.sourcingTypes})`,
      html: `
        <div style="font-family: sans-serif; max-width: 640px; color: #111;">
          <h2 style="margin-bottom: 16px;">New sourcing questionnaire</h2>
          <table style="border-collapse: collapse; font-size: 14px; line-height: 1.5;">
            ${row('Name', safe.name)}
            ${row('Email', safe.email)}
            ${row('Company / site', safe.company)}
            ${row('Sourcing type(s)', safe.sourcingTypes)}
          ...
          ${block('Must-have criteria', safe.criteria)}
          ${block('Already tried', safe.tried)}
        </div>
      `,
    });
